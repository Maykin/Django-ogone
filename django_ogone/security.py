...
from .settings import SHA_IN_WHITELIST, SHA_OUT_WHITELIST
...
class OgoneSignature(object):
    ...
    >>> ogone = OgoneSignature(dict(ACCEPTANCE='a', ACCEPTURL='b'), hash_method='sha512', secret='c')
    >>> sorted_data = ogone._sort_data(ogone.data)
    >>> sorted_data
    [('ACCEPTANCE', 'a'), ('ACCEPTURL', 'b')]
    >>> pre_sign_string = ogone._merge_data(sorted_data)
    >>> pre_sign_string
    'ACCEPTANCE=acACCEPTURL=bc'
    >>> signed = ogone._sign_string(pre_sign_string)
    >>> signed
    '4E4E3F785F746B4FCF5E1AC3C42203BD99601015D034A6FFAD4119A2751EDF8263559936FA4ABECD5CF36E7B1F0E872F830F4EFA0CD60AB61917C46B58D6D373'
    >>> ogone.signature()
    '4E4E3F785F746B4FCF5E1AC3C42203BD99601015D034A6FFAD4119A2751EDF8263559936FA4ABECD5CF36E7B1F0E872F830F4EFA0CD60AB61917C46B58D6D373'
    ...
    def _filter_data(self, k, v):
        ...
        if not self.out and k.upper() not in SHA_IN_WHITELIST:
            valid = False

        if self.out and k.upper() not in SHA_OUT_WHITELIST:
            valid = False
